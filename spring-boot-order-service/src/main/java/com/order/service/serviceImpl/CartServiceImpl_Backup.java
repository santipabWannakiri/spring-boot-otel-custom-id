...
@Service
@Slf4j
public class CartServiceImpl_Backup implements CartService {
    ...
    @Autowired
    public CartServiceImpl_Backup(OpenTelemetry openTelemetry, RestTemplate restTemplate, CartRepository cartRepository, UserRepository userRepository, TextMapSetter<HttpHeaders> setter) {
        this.openTelemetry = openTelemetry;
        this.restTemplate = restTemplate;
        this.cartRepository = cartRepository;
        this.userRepository = userRepository;
        this.tracer = openTelemetry.getTracer(CartServiceImpl_Backup.class.getName(), "0.1.0");
        this.setter = setter;
    }
    ...
    private boolean validateProductStatusAndQuantity(int productId, int quantity) {
        ...
        Span child = tracer.spanBuilder(methodName).setParent(Context.current().with(parentSpanThreadLocal.get())).setSpanKind(SpanKind.CLIENT).startSpan();
        ...
        try (Scope scope = child.makeCurrent()) {
            ...
            child.end();
            CartServiceImpl_Backup.setSpan(child);
        }
    }
    ...
    private boolean deductQuantity(int productId, int quantity) {
        ...
        String methodName = Thread.currentThread().getStackTrace()[1].getMethodName();
        Span child = tracer.spanBuilder(methodName).setParent(Context.current().with(parentSpanThreadLocal.get())).setSpanKind(SpanKind.CLIENT).startSpan();
        child.setAttribute(SemanticAttributes.HTTP_METHOD, "PUT");
        try (Scope scope = child.makeCurrent()) {
            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_JSON);
            openTelemetry.getPropagators().getTextMapPropagator().inject(Context.current(), headers, setter);
